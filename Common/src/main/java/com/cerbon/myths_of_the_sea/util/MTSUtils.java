...
public final class MTSUtils {
    ...
    public static void breakShip(LivingEntity attacker, ServerLevel serverLevel, int radius){
        if(isModLoaded("valkyrienskies") && serverLevel.getGameRules().getBoolean(GameRules.RULE_MOBGRIEFING)){
            BlockPos center = attacker.getOnPos();
            ...
            for (int x = -radius; x <= radius; x++) {
                for (int y = -radius; y <= radius; y++) {
                    for (int z = -radius; z <= radius; z++) {
                        ...
                        // Only break blocks from a ship
                        if(VSGameUtilsKt.getShipObjectManagingPos(serverLevel, pos)==null || serverLevel.isEmptyBlock(pos)) continue;

                        serverLevel.destroyBlock(pos, true); // "true" drops items
                    }
                }
            }
        }
