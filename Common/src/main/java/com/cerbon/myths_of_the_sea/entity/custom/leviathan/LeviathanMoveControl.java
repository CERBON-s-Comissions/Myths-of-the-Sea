...
public class LeviathanMoveControl extends MoveControl {
    ...
    public LeviathanMoveControl(Mob mob, int maxTurnX, int maxTurnY, float inWaterSpeedModifier, float outsideWaterSpeedModifier, boolean applyGravity) {
        super(mob);
    ...
    public void tick() {
        ...
        if (this.operation == MoveControl.Operation.MOVE_TO && !this.mob.getNavigation().isDone()) {
            ...
            if (g < 2.5000003E-7F) {
                ...
                this.mob.setYRot(this.rotlerp(this.mob.getYRot(), h, 1));
                ...
                if (this.mob.isInWater()) {
                    ...
                    if (Math.abs(e) > 1.0E-5F || Math.abs(j) > 1.0E-5F) {
                        ...
                        k = Mth.clamp(Mth.wrapDegrees(k), (float)(-1), (float)1);
                        this.mob.setXRot(this.rotlerp(this.mob.getXRot(), k, 1));
                    }
    ...
    private static float getTurningSpeedFactor(float f) {
        return 0.5F - Mth.clamp((f - 10.0F) / 50.0F, 0.0F, 0.5F);
    }
}
